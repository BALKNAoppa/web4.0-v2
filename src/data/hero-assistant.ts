...
/**
 * ХӨНГӨН МЭДЭЭЛЭЛ — карт БИШ, нэр + тайлбарын богино мөрүүд, дотроо линктэй.
 *
 * Яагаад `offer` биш: `offer` нь ҮЙЛДЭХ картууд (үнэ, badge, товч) — тэр нь
 * ХУДАЛДАХ хэлбэр. Энэ нь ЧИГЛҮҮЛЭХ хэлбэр: үйлчилгээний хүрээг нэрлээд,
 * дэлгэрэнгүйг эзэн сайт руу нь дамжуулна. Карт бүрд товч тавибал
 * гурван удаа ижил газар руу заасан гурван товч болно.
 *
 * ⚠️ Линк нь МЭДЭЭЛЛИЙН ХЭСЭГТ, дотроо. Кейсийн түвшний `cta` нь хариултын
 * доор, голд, том товч болж гардаг — энэ хэлбэрт тэр нь хэтэрхий хүчтэй.
 */
export type NoticeResult = {
  kind: "notice";
  /** Жагсаалтын дээрх богино гарчиг */
  title?: string;
  /**
   * Нэр + нэг мөр тайлбар. Үнэ, badge, товч АЛГА.
   *
   * `href` өгвөл НЭР нь линк болно (товч БИШ) — мөр бүр өөр эзэнтэй байж
   * болох тул — жишээ нь чиглэл тус бүр өөрийн дэд хуудас руу.
   */
  items?: { name: string; hint: string; href?: string }[];
  /** Жагсаалтын доорх линк — жижиг, хүрээтэй (дүүрэн товч БИШ) */
...
export type OfferCard = {
  ...
  price?: string;
  /**
   * Үнийн ард ХААЛТАД орох жижиг тодотгол ("НӨАТ-гүй", "НӨАТ-тай").
   *
   * Яагаад бүлгийн доод тэмдэглэлд БИШ: НӨАТ нь тухайн ТООНЫ шинж чанар
   * тул үнэтэйгээ ЗЭРЭГЦЭЖ байх нь хамаарлыг илүү тодорхой болгоно.
   *
   * `planId` өгсөн карт нь `mobile-plans.ts`-ээс уншдаг бөгөөд тэр үнэ нь
   * НӨАТ-ГҮЙ тул автоматаар "НӨАТ-гүй" болно — энд бичих шаардлагагүй.
   */
  priceNote?: string;
  /**
...
/*
 * ⚠️ `CLARIFY_INTRO` УСТГАСАН. Өмнө нь эхний тодруулах асуултын дээр
 * "Таны хэрэгцээг зөв ойлгохын тулд хэдэн зүйл тодруулъя" гэсэн тогтмол мөр
 * гардаг байв. Гэтэл clarify кейс ХОЁУЛАА (`youth-new-plan`,
 * `univision-new-customer`) өөрсдийн `summary`-дээ ЯГ ТЭР УРИЛГЫГ аль хэдийн
 * агуулдаг ("...доорх асуултад хариулж өөрийн багцаа хамтдаа бүтээгээрэй") —
 * нэг зүйл ХОЁР УДАА, хоёр өөр өнгөөр хэлэгдэж байлаа.
 *
 * Урилга нь `summary`-д үлдэнэ: тэр бол захиалагчийн бичсэн эх бичвэр,
 * кейс бүрд өөр байх ёстой. Кодод бичигдсэн тогтмол мөр түүнийг ДАВТАНА.
 */
  ...
  return {
    ...
    cards: sorted.flatMap((id) => {
      ...
      return [
        {
          id: `youth-${plan.id}`,
          // `planId` — ҮНЭ ба НӨАТ-ийн тодотголыг `mobile-plans.ts`-ээс уншина.
          planId: plan.id,
          // ⚠️ Гарчиг нь БАГЦЫН НЭР, дата эрхийн тоо БИШ. Онцлохууд нь гэр
          // бүлийн ХҮРЭЭГ хэлдэг ("16GB-88GB дата эрх") тул тухайн шатны тоо
    ...
    // ⚠️ Бүлгийн доод тэмдэглэл АЛГА. НӨАТ-ийн тодотгол нь үнийн ард
    // хаалтад орсон (`priceNote`) — доор давтах нь шуугиан.
  };
}
...
const youthNewPlanCase: AssistantQuestion = {
  ...
  /**
   * ⚠️ Асуулт ХОЁР өгүүлбэр байсныг ("Таны гар утасны хэрэглээ хэр вэ?" ба
   * "Хэдэн зүйл тодруулъя.") ГАНЦ өгүүлбэр болгов — хэрэглэгчийн бичсэн
   * хувилбар. Хоёр өгүүлбэр нь нэг л зүйлийг хоёр удаа асууж байв.
   */
  summary:
...
const businessNexmindCase: AssistantQuestion = {
  ...
  // ⚠️ Кейсийн түвшний `cta` ХАСАГДСАН — линк нь одоо хариултын ДООД талд,
  // голд, том товч болж гарахаа болиод мэдээллийн хэсгийн ДОТОР орсон.
  result: {
    kind: "notice",
    title: "Nexmind-ийн үйлчилгээний чиглэл",
    /**
     * ⚠️ ЧИГЛЭЛИЙН НЭР нь БОДИТ: `navigation.ts`-д бүртгэлтэй Nexmind-ийн
     * тайлбараас ("Managed network, дата төв, IT шийдэл").
     *
     * ⚠️ Харин `hint` нь тухайн чиглэл ЮУ ХИЙДГИЙГ ерөнхийд тайлбарласан
     * PLACEHOLDER — бодит бүтээгдэхүүний нэр (SKU) БИШ. Nexmind-ийн жинхэнэ
     * каталог repo-д АЛГА тул үйл ажиллагааны төрлөөр л бичив. Каталог
     * гармагц эндхийг бодит үйлчилгээний нэрсээр солино.
     */
    items: [
      {
      ...
      {
        // Салбарын чиглэлүүд — хэрэглэгчийн заасан жагсаалт.
...
const familyCase: AssistantQuestion = {
  // ⚠️ `featured` БИШ: persona 4 (`univision-new-customer`) үүнийг ОРЛОВ.
  // Тэр нь байрны хэмжээг АСУУЖ, багц ба төхөөрөмжийг `wifi-options.ts` ба
  // `plans.ts`-ээс гаргадаг бол энэ нь гурван шийдлийг хөдөлгөөнгүй
  // харьцуулдаг — хоёулаа жагсвал нэг сэдэв хоёр удаа гарч байв.
  // Устгаагүй шалтгаан: `office-connection` ба `plan-difference` энэ рүү
  // `followUps`-оор заадаг. `featured` нь ЗӨВХӨН таниагүй асуултын
  // жагсаалтад нөлөөлдөг тул тэр холбоосууд хэвээр ажиллана.
